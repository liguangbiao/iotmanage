<template>
    <div>
        <p class="p1">我的工坊</p>
        <p style="margin-top: 20px;text-align: center" v-if="res.length == 0">暂未有所属工坊！</p>
        <a v-if="res.length > 0" class="mc-content-head clear" @click="jump(item.id)" v-for="item in res">
            ...
            <div class="mc-content-head_right">
                <p><span class="organize_titie">{{item.teamName}}
                    <strong style="font-size: 14px; margin-left: 13px; font-weight: lighter;position: absolute; right: 7px;">【{{item.userIdCreate == user? "自建工坊":"参与工坊"}}】</strong></span>
                </p>

                <p><span>负责人：{{item.userNameCreate}}</span>

                </p>
                <p><span>工坊简介：</span><span>{{item.description}}</span></p>
                <!-- 当权限为工坊管理员时，显示此图标-->
...
<script>
    export default {
        ...
        methods:{
            getData(){
                let s = '/teamproject/belongworkshop';
            ...
            jump(id){
                this.$router.push({path:`/index/teamDetail/${id}`})
            }
        }
    }
</script>
